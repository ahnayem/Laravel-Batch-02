...
$feedbackLInk = 'http://'.$server.'/feedback/' . $user['shorten_code'];


// get data from feedback.json
$feedbackFile = 'feedback.json';
$feedbackData = file_exists($feedbackFile) ? json_decode(file_get_contents($feedbackFile), true) : [];
$feedback = isset($feedbackData[$email]) ? $feedbackData[$email] : [];

$feedback_html = '';

if(empty($feedback)) {
    $feedback_html = '<p class="text-gray-500">No feedback received yet.</p>';
} else{
    $feedback = array_reverse($feedback);

    foreach ($feedback as $key => $f) {
        $feedback_html .= '<div class="relative flex items-center space-x-3 rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:border-gray-400">
        <div class="focus:outline-none">
            <p class="text-gray-500">'.$f.'</p>
        </div>
    </div>';
    }
}
...
<header class="bg-white">
    <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end">
            <span class="text-sm font-semibold leading-6 text-gray-900"><?= $user_name ?></span>
        </div>
    </nav>
        ...
        <div class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div class="mt-6 flow-root">
                <div class="-my-6 divide-y divide-gray-500/10">
                    <div class="py-6">
                        <span class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50"><?= $user_name ?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="relative flex min-h-screen overflow-hidden bg-gray-50 py-6 sm:py-12">
        ...
        <div class="relative max-w-7xl mx-auto">
            <div class="flex justify-end">
                <span class="block text-gray-600 font-mono border border-gray-400 rounded-xl px-2 py-1">Your feedback form link: <strong><?= $feedbackLInk ?></strong></span>
            </div>
